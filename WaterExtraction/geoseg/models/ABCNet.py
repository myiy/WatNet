...
class Output(nn.Module):
    def __init__(self, in_chan, mid_chan, num_classes, up_factor=32, *args, **kwargs):
...
#         self.resnet = timm.create_model('resnet50', features_only=True, output_stride=32,
...
#         self.arm16 = AttentionEnhancementModule(512, 256)  # resnet50的layer2输出512通道
#         self.arm32 = AttentionEnhancementModule(1024, 256)  # resnet50的layer3输出1024通道
#         self.conv_head32 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
#         self.conv_head16 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
#         self.conv_avg = ConvBNReLU(2048, 256, ks=1, stride=1, padding=0)  # resnet50的layer4输出1024通道
#
...
class ContextPath(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.resnet = timm.create_model('resnet50', features_only=True, output_stride=32,
                                        out_indices=(1, 2, 3, 4), pretrained=pretrained)
        self.arm16 = AttentionEnhancementModule(1024, 256)  # resnet50的layer2输出512通道
        self.arm32 = AttentionEnhancementModule(2048, 256)  # resnet50的layer3输出1024通道
        self.conv_head32 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
        self.conv_head16 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
        self.conv_avg = ConvBNReLU(2048, 256, ks=1, stride=1, padding=0)  # 正确反映resnet50的layer4输出2048通道
    ...
    def forward(self, x):
        # 根据resnet50的features_only=True模式，适当处理所有返回的特征层输出
...
class ABCNet(nn.Module):
    def __init__(self, band=3, num_classes=8, pretrained=True):
        super(ABCNet, self).__init__()
        self.name = 'ABCNet'
        self.cp = ContextPath(pretrained)
        self.sp = SpatialPath()
        self.fam = FeatureAggregationModule(256 + 128, 256)
        self.conv_out = Output(256, 256, num_classes, up_factor=8)
        if self.training:
            self.conv_out16 = Output(256, 128, num_classes, up_factor=8)
            self.conv_out32 = Output(256, 128, num_classes, up_factor=16)
        self.init_weight()
...
if __name__ == '__main__':
    ...
    # flops:  133889527808.0 params:  33452864.0
