...
class LSRFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_h, x_w = x_h.permute(0, 2, 3, 1).contiguous(), x_w.permute(0, 2, 3,
                                                                     1).contiguous()  # [1, 16, 62, 128] [1, 16, 128, 62]
        ...
        # bhwc
        local_windows = window_reverse(local_windows, [4, 4], x_reduction.shape[1],
                                       x_reduction.shape[2]).contiguous()  # torch.Size([1, 128, 128, 16])
...
class Model(nn.Module):
    def __init__(self, num_classes=6, pretrained=True):
        ...
        self.convout = nn.Conv2d(config[0], num_classes, kernel_size=1, stride=1, padding=0)
