...
class RSAM(nn.Module):
    def __init__(self, channels, ext=2, r=16):
        ...
        self.spatial_mlp = nn.Sequential(nn.Linear(channels * scale, channels), nn.ReLU(),
                                         nn.Linear(channels, channels))
        self.spatial_att = ChannelAtt(channels * ext, channels)
        self.context_mlp = nn.Sequential(
            *[nn.Linear(channels * scale, channels), nn.ReLU(), nn.Linear(channels, channels)])
...
class IFM(nn.Module):
    ...
    def forward(self, x):
        ...
        df = rearrange(df, 'b h (hh ws1 ww ws2) d -> (b hh ww) h (ws1 ws2) d', h=self.num_heads, d=C // self.num_heads,
