...
def get_training_transform():
    train_transform = [
        ...
        albu.RandomRotate90(p=0.5),                 # 随机旋转90度
        albu.RandomBrightnessContrast(p=0.5),       # 随机调整图像的亮度和对比度
        albu.Normalize()                            # 归一化操作
    ]
    return albu.Compose(train_transform)


# 用于对输入的图像 (img) 和掩码 (mask) 进行数据增强
...
# 返回一个用于验证集（或测试集）的图像数据预处理的转换操作
def get_val_transform():
    val_transform = [
        albu.Normalize()        # 对图像进行归一化操作
    ]
    return albu.Compose(val_transform)


# 对输入的验证集图像 (img) 和掩码 (mask) 进行预处理
...
class LoveDATrainDataset(Dataset):
    def __init__(self, data_root='data/LoveDA/Train', img_dir='images_png', mask_dir='masks_png_convert',
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        ...
        urban_img_ids = [(str(id.split('.')[0]), 'Urban') for id in urban_mask_filename_list]
        ...
        rural_img_ids = [(str(id.split('.')[0]), 'Rural') for id in rural_mask_filename_list]
