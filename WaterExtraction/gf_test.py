...
def main():
    ...
    for class_name, class_iou, class_f1, class_precision, class_recall in zip(config.classes, iou_per_class,
                                                                              f1_per_class, precision_per_class,
                                                                              recall_per_class):
        print('F1_{}:{}, IOU_{}:{}, Precision_{}:{}, Recall_{}:{}'.format(class_name, class_f1, class_name, class_iou,
                                                                          class_name, class_precision, class_name,
                                                                          class_recall))

    print('F1:{}, mIOU:{}, Precision:{}, Recall:{}, OA:{}, Kappa:{}'.format(np.nanmean(f1_per_class[:-1]),
                                                                            np.nanmean(iou_per_class[:-1]),
                                                                            np.nanmean(precision_per_class[:-1]),
                                                                            np.nanmean(recall_per_class[:-1]), OA,
                                                                            kappa))
